...
The driving conformance path and the public “Drive with friends” composition now use the production replacement. The public path negotiates a configurable ruleset: the host pauses, selects a registered map and Cruise or Chase, publishes a repeated authoritative configuration epoch, waits for every connected client to acknowledge the loaded world, resets vehicles to the validated grid, and resumes. Chase shares presentation-free police mechanics with local play while the host exclusively owns AI targets, collisions, capture, survival time, reinforcements, and round resets.

Both paths use the same production mechanics: `AuthoritativeDrivingSimulation` creates one extracted production vehicle core per admitted player, maps bounded client control intent, assigns deterministic non-overlapping City Circuit spawns, resets both vehicles authoritatively when players collide, and publishes presentation-complete snapshots through `HostRuntime` and `ClientRuntime`. Clients render from a bounded tick-based interpolation buffer with a stable host-tick clock estimate so packet-arrival jitter cannot move render time backward, and a render-only `VehicleView`/fleet owns remote car meshes without simulation authority. The intentionally small pilot schema remains only as a protocol fixture.
...
- while paused, the host may select a registered multiplayer map and Cruise or Chase;
- configuration epochs clear stale interpolation, synchronize rendered-world and mode replacement, and reset camera tracking rather than easing across transitions;
