...
The homepage at `/` is reserved for the real-time demo introducing the game. The driving game is served at `/drive/`, including a separate Direct Invite “Drive with friends” Cruise mode with host-authoritative pause and synchronized map changes. Legacy root multiplayer and direct-invite links are forwarded to `/drive/`. Its private drivetrain tuning surface is at `/dyno/`, and Worker APIs live below `/api/`.
